...
- [`docs/features/cmux-mode.md`](docs/features/cmux-mode.md) — `--cmux` flag, four-pane scaffold (orchestrator hidden + header/log/footer), workspace-name composition and sanitization, Phase 2 real-workflow lifecycle (preflight → readiness handshake → workflow run → WorkspaceDone broadcast → DoneAck wait → dismiss → teardown), per-pane rendering (header checkboxes/iteration counter, log streaming, footer status-line/shortcuts/version/help-expand), minimum-size advisory, failure paths (handshake timeout, pane exit, orchestrator crash), log-artifact equivalence, five distinguishable preflight failures, dismissal-gesture coverage, and out-of-scope list for Phase 2
...
- [`docs/how-to/setting-up-cmux.md`](docs/how-to/setting-up-cmux.md) — Install cmux v0.64.6, verify descendants-only access mode and allow-all caveat, run pr9k with `--cmux` against a real workflow, Phase 2 walkthrough (what each pane shows, navigating/scrolling the log pane, `?` help expand, `q`/`y` quit, verifying log artifacts), workspace-dismissal gestures (workspace-close and close-each-pane), and orphan-workspace recognition via the `pr9k-` prefix
