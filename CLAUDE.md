...
- [`docs/features/statusline.md`](docs/features/statusline.md) — Status-line package reference: `Runner` goroutine lifecycle, `State` snapshot, `BuildPayload` stdin JSON, `Sanitize` ANSI output filtering, single-flight exec, 8 KB stdout limit, full-env trust model, and cold-start / shutdown ordering
