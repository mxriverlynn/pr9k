...
- [`docs/code-packages/cmuxctl.md`](docs/code-packages/cmuxctl.md) — `internal/cmuxctl`: `CmuxClient` interface (12 RPC methods, including Phase 4 `WorkspaceSetStatus`/`WorkspaceClearStatus`/`WorkspaceSetProgress`/`WorkspaceClearProgress`), `RealClient` single-goroutine sequential queue with per-call timeout, `FakeClient` test double with scripted responses and hang injection, `Preflight` five-condition check with `CMUX_SOCKET_PATH` validation, `RunPhase1` workspace lifecycle, `DismissalObserver` 500ms polling, and `SanitizeBasename`
