...
- [`docs/features/cmux-mode.md`](docs/features/cmux-mode.md) — `--cmux` flag, four-pane scaffold (orchestrator hidden + header/log/footer), workspace-name composition and sanitization, Phase 2 real-workflow lifecycle (preflight → readiness handshake → workflow run → WorkspaceDone broadcast → DoneAck wait → dismiss → teardown), per-pane rendering (header checkboxes/iteration counter, log streaming, footer status-line/shortcuts/version/help-expand), Phase 3 interactive error recovery (continue/retry/quit from footer pane, header `[✗]` marker, log retry-separator ordering, control-key absorption on non-control panes), Phase 4 sidebar mirroring (`cmuxSidebar` adapter, status pill `pr9k.step`, progress bar for bounded runs, error-mode suffix `— awaiting input`, graceful-path clear via `ClearAll`), Phase 5 lifecycle notifications (three classes: completion/run-aborted/error-mode, 60-second persistent re-fire for error-mode, dismissal-resistance, first-keystroke timer-stop via `IntentErrorQuitInitiated`/`IntentErrorQuitCancelled`, `WorkspaceNotify` preflight probe), Phase 6 robust failure handling (abort gate, liveness signal + 45 s stall threshold, four detection paths: display-loss/pane-close/channel-stall/cmux-error, `run aborted` token rendered in each pane, orchestrator-death behavior, post-run window guard), minimum-size advisory, failure paths (handshake timeout, pane exit, orchestrator crash), log-artifact equivalence, five distinguishable preflight failures, dismissal-gesture coverage, and out-of-scope lists for Phase 5 and Phase 6
...
- [`docs/code-packages/logger.md`](docs/code-packages/logger.md) — `internal/logger`: Concurrent-safe timestamped file logger with millisecond-precision filenames and `RunStamp()` accessor for artifact directory naming; `CompareRunDirs` test-only artifact equivalence comparator (D-13)
...
- [`docs/code-packages/atomicwrite.md`](docs/code-packages/atomicwrite.md) — `internal/atomicwrite`: `Write(path, data, mode)` atomic file replacement via temp-file + rename, symlink-preservation, ENOENT first-save walk, EXDEV propagation, and parent-directory fsync
- [`docs/code-packages/ansi.md`](docs/code-packages/ansi.md) — `internal/ansi`: `StripAll(b []byte) []byte` strict ANSI escape sequence stripper (CSI, OSC, bare ESC, two-byte sequences); used for the workflowio load recovery view
- [`docs/code-packages/workflowmodel.md`](docs/code-packages/workflowmodel.md) — `internal/workflowmodel`: `WorkflowDoc`, `Step`, `StepKind`, `EnvEntry`, `StatusLineBlock`, `DefaultsBlock` types; `IsDirty` diff (ignores UnknownFields); `Empty` scaffold; `CopyFromDefault` bundle reader; `DefaultScaffoldModel` and `ModelSuggestions`
...
- [`docs/code-packages/workflowedit.md`](docs/code-packages/workflowedit.md) — `internal/workflowedit`: Bubble Tea Model for the workflow-builder TUI; exported API (Model, New, EditorRunner, ExecCallback, DialogKind, PickerKind, constants); Update routing (pre-dispatch → help → dialog → global keys → edit view); outline row kinds, detail field kinds (text/choice/numeric/modelSuggest/secretMask), save pipeline, conflict detection, session-event logging, load-pipeline banner forwarding, Bubble Tea ctx exemption (D-PR2-13); Visual Layout section with per-surface render file map and dialog/field render grammars
- [`docs/code-packages/uichrome.md`](docs/code-packages/uichrome.md) — `internal/uichrome`: shared chrome primitives for both run-mode and workflow-builder TUIs; `WrapLine`, `HRuleLine`, `BottomBorder`, `RenderTopBorder` border helpers; `Overlay`/`SpliceAt` ANSI-safe overlay utilities; color palette (`LightGray`, `White`, `Green`, `Red`, `Yellow`, `Cyan`, `Dim`); geometry constants (`MinTerminalWidth`=60, `MinTerminalHeight`=16, `DialogMaxWidth`, `DialogMinWidth`, `HelpModalMaxWidth`)
- [`docs/code-packages/cmuxctl.md`](docs/code-packages/cmuxctl.md) — `internal/cmuxctl`: `CmuxClient` interface (13 RPC methods, including Phase 4 `WorkspaceSetStatus`/`WorkspaceClearStatus`/`WorkspaceSetProgress`/`WorkspaceClearProgress` and Phase 5 `WorkspaceNotify`), `RealClient` single-goroutine sequential queue with per-call timeout, `FakeClient` test double with scripted responses and hang injection, `Preflight` five-condition check with `CMUX_SOCKET_PATH` validation and Phase 5 `WorkspaceNotify` probe, `RunPhase1` workspace lifecycle, `DismissalObserver` 500ms polling, and `SanitizeBasename`; Phase 5 additions: `WorkspaceNotify` (`notification.create_for_target` wire method), `NotificationClass` typed constants, `*TimeoutError`, `IntentErrorQuitInitiated`/`IntentErrorQuitCancelled`
- [`docs/code-packages/interactionchannel.md`](docs/code-packages/interactionchannel.md) — `internal/interactionchannel`: Phase 2 Unix-socket IPC between orchestrator and display panes; `Channel` (Serve/Dial/AwaitReady/SendState*/Close), `Message` interface and concrete types (Ready/Intent/DoneAck/StateHeader/StateLog/StateFooter/WorkspaceDone), `IntentType` enum, length-prefixed JSON framing with `"type"` discriminator, per-connection goroutine model (read+write+watcher), per-role buffered channels with drop-oldest log semantics, handshake mutex, ack tracker, and three test doubles (`FakeInteractionChannel`, `FakeDisplayPane`, `FakeFooterKeySource`)
...
- [`docs/adr/20260424120000-workflow-builder-save-durability.md`](docs/adr/20260424120000-workflow-builder-save-durability.md) — Atomic temp-file+rename for all user-facing writes, companion-first ordering (D-20), nanosecond mtime conflict detection (D-41), crash-temp detection (D-16), and EXDEV/ENOSPC/EACCES classification in `workflowio`. Apply when modifying file-write code in `workflowio` or `atomicwrite`, or evaluating proposals to write user files without atomic replacement.
...
- [`docs/coding-standards/file-writes.md`](docs/coding-standards/file-writes.md) — File-write rules: use `atomicwrite.Write` for durable replacement, `O_CREATE|O_EXCL|O_WRONLY, 0o600` for initial temp files, prohibition on `O_TRUNC` in new code (with documented exemptions), and prohibition on `os.WriteFile` for externally sourced paths. Apply when writing any code that creates or replaces files on disk.
- [`docs/coding-standards/tui-rendering.md`](docs/coding-standards/tui-rendering.md) — TUI rendering standards: plain-text-first ANSI composition (single styling pass), `lipgloss.Width()` for visual width, generation counter for stale async banner rejection, and render file decomposition (`render_*.go` per component). Apply when writing or reviewing any Bubble Tea render function.
...
- [`docs/how-to/setting-claude-effort.md`](docs/how-to/setting-claude-effort.md) — Per-step `effort` field and the top-level `defaults.effort` block, valid values (`low`/`medium`/`high`/`xhigh`/`max`), resolution rules, and validator constraints
...
- [`docs/how-to/setting-up-cmux.md`](docs/how-to/setting-up-cmux.md) — Install cmux v0.64.7, verify descendants-only access mode and allow-all caveat, run pr9k with `--cmux` against a real workflow, Phase 2 walkthrough (what each pane shows, navigating/scrolling the log pane, `?` help expand, `q`/`y` quit, verifying log artifacts), recovering from a failed step (continue/retry/quit from footer pane, `[✗]` header marker, retry separator in log), workspace-dismissal gestures (workspace-close and close-each-pane), and orphan-workspace recognition via the `pr9k-` prefix
