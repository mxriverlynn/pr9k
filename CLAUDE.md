...
- [`docs/features/sandbox-subcommand.md`](docs/features/sandbox-subcommand.md) — `sandbox create` subcommand and `sandbox --interactive` (`-i`) flag: Docker check, image pull, smoke test with ANSI sanitization for create; interactive claude REPL with auto-pull and profile-dir auto-create for `--interactive`; shared helpers and dependency injection design
