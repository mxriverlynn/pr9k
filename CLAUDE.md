...
- [`docs/features/cmux-mode.md`](docs/features/cmux-mode.md) — `--cmux` flag, four-pane scaffold (orchestrator hidden + header/log/footer with Phase 1 placeholder labels), workspace-name composition and sanitization, lifecycle (preflight → create → spawn → dismiss → teardown with focus restore), five distinguishable preflight failures, dismissal-gesture coverage, and out-of-scope list for Phase 1
...
- [`docs/how-to/setting-up-cmux.md`](docs/how-to/setting-up-cmux.md) — Install cmux v0.64.6, verify descendants-only access mode and allow-all caveat, run pr9k with `--cmux`, workspace-dismissal gestures (workspace-close and close-each-pane), and orphan-workspace recognition via the `pr9k-` prefix
