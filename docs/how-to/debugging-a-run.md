...
If pr9k is still running, start with the log panel — scroll back with `↑`/`k`/`↓`/`j` in Normal or Done mode. If pr9k has exited, open the log file at `<project-dir>/.pr9k/logs/`.

> **Tip:** Logs land under `<project-dir>/.pr9k/logs/`. Add the runtime-state entries to your repo's `.gitignore` before your first run to prevent log files from appearing as untracked changes:
> ```
...
These files live in the project working directory and are not committed. If a run fails partway through, they may be left behind — a leftover `test-plan.md` means "test writing didn't run or didn't finish", not "here's your plan". Delete them (or let the next run overwrite them) before reproducing.
