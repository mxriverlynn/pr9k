...
An opt-in launch mode activated by `--cmux`. When set, `main()` bypasses the normal TUI workflow entirely and runs a five-condition `cmuxctl.Preflight` check followed by `cmuxctl.RunPhase1`, which creates a recognisably-named cmux workspace with three surfaces (log, header, footer), runs the full workflow via `runCmuxWorkflowAdapted`, holds it open via a 500 ms dismissal-observation polling loop, and tears it down cleanly with prior-workspace focus restore. The pr9k process the operator launched **is** the orchestrator (Architecture A — no hidden pane). SIGINT, SIGTERM, and SIGHUP all route through the same teardown path. The workspace name is composed as `pr9k-<sanitized-basename>-<nanosecond-timestamp>` and printed to stdout on creation. During the run, the `cmuxSidebar` adapter mirrors the current step name and iteration progress into cmux's workspace sidebar via `WorkspaceSetStatus` / `WorkspaceSetProgress`; both entries are cleared at graceful run-end via `ClearAll`.
...
Validates `config.json` against all ten D13 categories in a single pass, collecting every error before returning. Checks file presence and parseability, per-step schema shape (including `isClaude`, `captureAs`, `breakLoopIfEmpty`, the `effort` value set with claude-only enforcement, and the effective-model requirement for claude steps), phase size, referenced file existence, and variable scope resolution. Also validates the top-level `env` array (Category 10) and enforces sandbox isolation rules B and C (host-path tokens in prompts, and captureAs+host-path in commands; Rule A was removed in issue #91 — captureAs on claude steps is now valid and binds via the Aggregator). Validates the optional top-level `statusLine` block (command resolvability, `refreshIntervalSeconds` range, unknown-field rejection) and the optional top-level `defaults` block (`effort` validated against the same value set as the per-step field; `model` accepted as any non-empty string and used as the effective-model fallback). Returns a slice of structured `Error` values; an empty slice means valid. Wired into `main.go` immediately after `steps.LoadSteps`; validation failures exit 1 with structured errors on stderr before the TUI starts.
...
cmd/pr9k/workflow.go                (pr9k workflow subcommand)
    ├── internal/logger
    └── internal/workflowedit

internal/workflowedit          (Bubble Tea Model for the workflow-builder TUI)
    ├── internal/workflowio
    ├── internal/workflowmodel
    ├── internal/workflowvalidate
    └── internal/ansi

internal/atomicwrite           (durable temp-file + rename writer)
internal/ansi                  (StripAll for untrusted bytes)
internal/workflowmodel         (in-memory WorkflowDoc types)
internal/workflowio            (load/save/detect)
