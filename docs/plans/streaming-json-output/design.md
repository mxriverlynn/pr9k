...
- **Non-breaking on current workflow:** Verified against `src/config.json` — no `isClaude: true` step currently declares `captureAs`. The behavior change alters nothing observable in the default workflow today.
...
- **Behavior:** No new field in config.json. `BuildRunArgs` always appends `--output-format stream-json --verbose`. The JSON pipeline activates whenever `IsClaude == true`.
...
- **Version bump:** Patch-level per `docs/coding-standards/versioning.md` — no change to CLI flags, `config.json` schema, `{{VAR}}` language, or `--version` output. Internal "how we read claude" is implementation detail.
...
- config.json schema: unchanged. (D17)
...
| `captureAs` for claude steps is currently used | Secondary | **Refuted** (verified against `src/config.json`) — no claude step sets `captureAs`, so D6 is a no-op for the default workflow | D6 annotated as non-breaking; no validator change required |
...
Re-read against the current state of the codebase (`workflow.go`, `run.go`, `orchestrate.go`, `log_panel.go`, `logger.go`, `logger_test.go`, `config.json`, `validator.go`) to catch any drift introduced by the earlier wiring descriptions.
