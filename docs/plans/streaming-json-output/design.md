...
- **Edge case — retry after is_error:** On retry, a fresh `Aggregator` and `RawWriter` are constructed for the new `RunSandboxedStep` invocation. The prior attempt's `StepStats` are **not** added to `RunStats` (the failing attempt's tokens still represent real spend, but attributing them to a retried step would double-count the summary line against the successful attempt). Open question O-2 below records this as a deliberate simplification to revisit.
...
### D21. Continue-past-failure (`c`) leaves the failed-attempt JSONL in place and counts tokens
- **Behavior on `c` (continue):** The failed step's JSONL file is preserved as-is (it contains whatever events arrived before the failure). The step's `StepStats` **is** folded into `RunStats` — failed attempts still consumed tokens and that spend is real. The cumulative summary line at finalize wording is `cumulative across all claude step attempts` so readers understand the total includes failed attempts.
- **Behavior on `r` (retry):** The JSONL file is truncated (D14) because the new attempt is the definitive record for that step slot. The previous attempt's `StepStats` is **not** folded; only the successful attempt's (or the last attempt's, if all retries fail and the user eventually presses `c`) is counted. Rationale: avoid double-counting when the user iterates toward success; accept that failed retries under-report slightly. O-2 captures the alternative.
- **Summary-line semantics:** The finalize summary line explicitly labels the total as `successful + continued-past-failure attempts` so the number is self-describing.
...
- **O-1. Does `--permission-mode bypassPermissions` coexist cleanly with `--output-format stream-json --verbose`?** Evidence: both flags are documented in claude CLI's `cli-reference` as independent flags. No documented interaction. Plan assumes independence. If `bypassPermissions` suppresses some `system` init field (e.g., tool list), the parser already ignores unknown-typed lines (D7/D8). **Resolution path:** first end-to-end test run against a real claude invocation will surface it.
- **O-2. Should a failed attempt's `StepStats` feed the cumulative `RunStats`?** Plan currently discards failed-attempt tokens from the cumulative summary (D15 retry edge case). This slightly under-reports real Anthropic spend when retries happen. If per-run cost precision ever becomes important, the fix is to sum all attempts and have the summary line distinguish "attempted" from "successful" tokens. Deferred until a consumer asks for it.
- **O-3. Filename collisions across workflow edits.** If a user edits `ralph-steps.json` mid-run to rename a step (very unlikely — the runner loads once at startup), the NN index is stable but the slug changes. Non-issue in practice because `LoadSteps` runs once; documenting the invariant keeps the contract crisp.
- **O-4. Per-step tolerance for `error_max_turns`.** D15 treats all `is_error: true` as failure. In practice, `error_max_turns` may carry a usable partial answer that some workflows would prefer to accept. If this hits the default ralph workflow, add a `tolerateMaxTurns: true` field on the step schema; until then, the `c` continue-past-failure path (D21) covers the workaround (user continues and downstream step sees an empty `{{CAPTURE}}`).
- **O-5. `session_id` threading for multi-step claude workflows.** If a future workflow wants step 2 to resume step 1's session, the exact touchpoints are: (a) expose the captured `session_id` via a new optional `captureSessionAs` field on the step, (b) teach `BuildRunArgs` to append `--resume <id>` when `{{SESSION_ID_FROM_PREV}}` is substituted into a prompt. Neither touchpoint exists today. Trigger to revisit: first workflow that needs cross-step memory.
