...
- **Technical detail:** The state file is a single JSON object, atomic-written via `atomicwrite.Write` (the same pattern `workflowio.Save` uses for `config.json`). The file is the deterministic marker for "is there an active or recoverable pr9k run on this primary?" — its presence means yes, its absence means no.
  ...
  **Lifecycle ordering (load-bearing for [T1](feature-technical-notes.md#t1-sequencing-constraint)):**
  ...
  - Write happens *inside* startup, after `preflight.Run`. Under [T1](feature-technical-notes.md#t1-sequencing-constraint)'s redirect, `preflight.Run` runs against the worktree path and creates `<worktree>/.pr9k/` — it does **not** create `<primary>/.pr9k/`. The state-file write therefore takes responsibility for ensuring its own parent directory exists: it calls `os.MkdirAll(<primary>/.pr9k/, 0o755)` immediately before invoking `atomicwrite.Write` (the temp-file open used by `atomicwrite.Write` cannot create a missing parent directory — the underlying `os.OpenFile` returns ENOENT in that case). On a primary that has never had a pr9k run before, this MkdirAll is the only thing creating `<primary>/.pr9k/`. Write happens only when `worktrees.enabled: true`. (Review F31.)
...
- **Technical detail:** D17 commits to one append-only `iteration.jsonl` per worktree. The default workflow's existing consumers — `workflow/scripts/post_issue_summary` (line 11: hardcoded `JSONL_FILE=".pr9k/iteration.jsonl"`) and `workflow/prompts/lessons-learned.md` (lines 1, 5, 8: hardcoded `.pr9k/iteration.jsonl`) — read a literal relative path and have no mechanism to receive an invocation-stamp at runtime. The naive expectation that consumers could derive the path from `RunStamp()` (a Go-runtime value) is false for shell scripts and prompts (review F3). Keeping the path identical to today's, and keeping the file's content shape identical (one `IterationRecord` per line), means every existing consumer keeps working unchanged.
  ...
  4. **Record schema extension:** each `IterationRecord` written by this feature carries a new `invocation_stamp` field set to the current invocation-stamp. The behavioral commitment is that the field is present on every record this feature writes — implementation is responsible for setting it at the single record-construction layer (today's `newIterationRecord`-equivalent in `internal/workflow`), not at every individual call site, so no future call path can forget it. Older records (written by an earlier pr9k build that pre-dated this feature, or written by a downgraded build) may lack the field; the **default-workflow consumers** (`post_issue_summary`, `lessons-learned`) tolerate its absence — `post_issue_summary` filters by named fields with `jq` and silently treats missing fields as empty, and `lessons-learned` reads the file as an unstructured corpus. The tolerance guarantee is scoped to the named default consumers; user-written consumers that filter by `invocation_stamp` must accept that older records may not carry it. The field is informational — no consumer in the default workflow filters by it.
  ...
  6. **Truncation:** the default workflow's `lessons-learned` step continues to truncate the file at the end of finalization (today's behaviour, see `workflow/prompts/lessons-learned.md` line 8). This is the bound that prevents unbounded growth across many resumed runs into the same worktree — but the bound applies *only when finalization runs to completion*. A user who repeatedly kills the run before reaching `lessons-learned`, or a custom workflow that omits the step, will see the file grow across resumed invocations until the worktree is removed (autoCleanup or `pr9k worktree prune`). The growth is per-iteration-step (typically a few hundred bytes per record), so absolute size remains modest in normal use; the more user-visible effect is on `post_issue_summary`, which reads the entire file with no `issue_id` filter and may include cross-invocation, cross-issue records in a comment (see the "Resumed `post_issue_summary` includes records from prior invocations" Edge Cases row).
