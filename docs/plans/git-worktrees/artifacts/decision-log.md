...
- **Rationale:** This is a reversal of an earlier draft of D17 that committed to per-invocation files (`iteration-<invocation-stamp>.jsonl`) with a stable alias at `iteration.jsonl`. User testing of that model surfaced the regression: when a user quits pr9k mid-run before `lessons-learned` runs, today's append-only model preserves the killed invocation's records into the resumed run — `lessons-learned` and `post_issue_summary` both see the full set. The per-invocation-file model would have hidden those records behind the alias (which only points at the current invocation), so the resumed run's `lessons-learned` would see only its own records and lose the killed invocation's forensic value. The user's stated requirement is "I want the worktree to also be able to see all of the iterations from a previous run, if `Lessons learned` step never executes" — which is satisfied exactly by the today-style append-only model. The previously-feared correctness problem ("consumers cannot tolerate mixed records") is not actually a problem under the worktree scope: today's `post_issue_summary` already accumulates records across iterations within a single run (the script's own comment documents this: "issue N's comment may include iterations 1..N"), and `lessons-learned` is *designed* to look across iterations. Worktree-scoped append-only is just today's behavior, with the worktree as the cleanup boundary instead of the projectDir.
