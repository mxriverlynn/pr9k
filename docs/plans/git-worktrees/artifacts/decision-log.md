...
- **Decision:** A brand-new branch named `pr9k-<worktree-stamp>` created off the primary's HEAD at worktree-creation time. The worktree-stamp is the millisecond-precision stamp pr9k already uses for its log directory (so the branch, the worktree directory's basename suffix, and the worktree-stamp are the same identifier). Branch names use the **hyphen** separator, not a slash — see review F1 in `review-findings.md`.
- **Rationale:** Git enforces "one branch can be checked out in only one worktree at a time" (`../investigation/01-worktree-mechanics.md` E1), so the worktree must be on a different branch than whatever the primary has checked out. Creating a fresh branch off the primary's HEAD gives pr9k a clean starting point with no risk of conflict. The `pr9k-` prefix makes branches discoverable with `git branch --list 'pr9k-*'` and removable with the same pattern. The worktree-stamp is unique by construction. The hyphen separator was chosen over a slash because (a) D5's stale-detection now filters by directory-name pattern only — the slash form's `git branch` namespacing is no longer load-bearing for any spec behaviour (see review F14), and (b) the hyphen form is consistent across the spec body, T4 schema, and the cleanup commands in D13.
...
- **Driven by findings:** —
- **Dependent decisions:** D5 (stale-worktree detection filters by directory-name pattern; `pr9k-` branch prefix is a convention only), D7 (first push needs upstream because the branch is fresh).
...
- **Decision:** Logs and artifacts live inside the worktree's `.pr9k/logs/` exactly as they do today in the in-place model. The worktree itself is left on disk after the run by default ([D4](#d4-cleanup-policy)), so the logs persist. The worktree path, the primary path, and the run's branch name are written to the run's log file (header) so post-run users can grep that file to learn which worktree a run used without depending on TUI history. (Earlier drafts also wrote a "first record" of the same fields to the iteration log; that commitment was withdrawn in review F33 because the iteration-log consumer `post_issue_summary` reads every line through a jq filter that expects per-step `IterationRecord` shape — a non-step record would corrupt the issue comment. Per-invocation attribution inside the iteration log is carried by the `invocation_stamp` field on every per-step record instead — see [D17](#d17-per-worktree-iteration-log) and [T5](feature-technical-notes.md#t5-per-worktree-iteration-log).)
...
- **Decision:** Three coordinated changes:
  ...
  2. The push step surfaces non-zero exit codes; on failure, pr9k enters its existing error-recovery mode (the c/r/q prompt) with the verbatim git stderr written to the iteration log and the run's log file. **This applies to both `worktrees.enabled: true` and `worktrees.enabled: false` modes** (resolved OI-1, 2026-05-08): the silent-failure mode is a pre-existing bug regardless of mode, and the c/r/q affordance is the right behavior for both. The fix lives in `workflow/scripts/git_push` and is unconditional — no per-mode branching.
...
- **Rationale:** Without upstream-setting, the first push of `pr9k-<worktree-stamp>` fails. Without surfacing the failure, pr9k closes the issue, prints "done," and the user is told everything succeeded — but no code is on the remote (validation finding V14 / E14; team finding F-02). This is a pre-existing bug that becomes a much louder problem in the worktree path because every `useWorktrees` run starts on a new branch with no upstream, and once the first push fails every subsequent push in the same run fails for the same reason. Routing failures into the existing error-recovery mode (rather than aborting silently or aborting hard) gives the user the standard c/r/q affordance and matches how every other failure in the workflow behaves.
...
- **Decision:** The active-run state file at `<primary>/.pr9k/active-run.json` (D15) doubles as a soft lock. On startup, if the state file exists and its recorded PID is alive AND the recorded binary path matches the running pr9k binary, the new invocation exits non-zero with "another pr9k appears to be running for this primary checkout (PID N). Stop the running process before re-running." `--fresh` does **not** override this guard (review F7) — it would otherwise destroy state belonging to a live process. If PID is alive but binary doesn't match, treat as PID-reuse-by-unrelated-process and resume normally. If PID is dead, resume normally.
...
- **Decision:** The user observes the worktree from two places:
  1. **The run's log file (header line)** records the worktree path, primary path, and branch name when the run starts. Post-run grep against this header line is the durable forensic surface that does not depend on the TUI being visible. (An earlier draft also wrote the same fields as a "first record" to the iteration log; that path was withdrawn in review F33 — see D3 and the F33 finding for the reason.)
  2. **The TUI** prefixes its iteration line with the worktree's basename during the run, and the final summary line shows the worktree path, branch name, and the count of all `pr9k-*` worktrees on disk.
...
- **Decision:** Yes. A new `pr9k worktree prune` subcommand is added. It resolves the primary directory from `--project-dir` if provided, otherwise from CWD; if the resolved path's basename matches the `<basename>-pr9k-*` pattern (the user invoked prune from inside a worktree), pr9k walks one level up and treats that ancestor as the primary (review F8). It then discovers pr9k-owned worktrees using the same path-prefix filter D5 uses (directory name matches `<primary-basename>-pr9k-*`), force-removes each (because pr9k worktrees routinely contain uncommitted intermediate workflow files), and deletes each `pr9k-<worktree-stamp>` branch. Before each removal, pr9k checks for uncommitted changes in the worktree (covering both modified tracked files **and** untracked files — the intermediate workflow artifacts the spec relies on, such as `progress.txt` and `test-plan.md`, are untracked by construction); the combined count is included in that worktree's per-removal feedback line so the loss is visible (review F12, F21). The removal still proceeds — `prune` is an explicit user action — and the warning satisfies the Invariants section's "no silent destruction" property for uncommitted files. A `--dry-run` flag lists what would be removed without changing anything; its output is not a transactional plan (any pr9k run that starts between dry-run and the real prune will correctly exclude its own active worktree). Per-worktree errors are surfaced verbatim and the subcommand continues with the rest; the exit code is non-zero if any removal failed. The subcommand is independent of the workflow config — it works whether or not `worktrees.enabled` is set, and even works on a primary checkout that never used the worktree feature (it simply finds no entries and exits zero).
...
- **Decision:** Yes. pr9k writes an active-run state file at `<primary>/.pr9k/active-run.json` at the start of every run with `worktrees.enabled: true`. The file contains the worktree path, branch name, worktree-stamp, PID, binary path, started-at timestamp, schema version, and pr9k version (T4). On every pr9k startup, the file is read; if it exists and points to a valid worktree on the same primary, AND the recorded process is no longer running (PID dead, or PID alive but binary doesn't match — the PID-reuse case), pr9k auto-resumes by entering the existing worktree on its existing branch. No new worktree is created, no new worktree-stamp is generated; only a fresh invocation-stamp is generated for log/artifact paths. The state file is removed at the end of a `Completed` or `LoopBroken` run (D16). On `UserQuit` it is kept (so the next invocation auto-resumes). On hard kills it is kept by virtue of nothing running. The `--fresh` CLI flag (D19) lets the user explicitly discard the in-flight state.
  ...
  Resume is **iteration-loop-level**, not step-level: the resumed run's iteration loop calls `get_next_issue` as it normally would. Because the script returns the lowest-numbered open pr9k-labeled issue and a prior run that died mid-iteration left its issue open (D7+D18 ensure push-before-close so closed-but-unpushed cannot happen), the in-flight issue is naturally re-delivered. The iteration runs again from feature-work onward on a branch that already carries any prior commits.
...
- **Dependent decisions:** D5 (active worktree excluded from stale detection), D7+D18 (push-before-close eliminates the silent-skip window resume must not hit), D10 (state file doubles as soft lock), D13 (autoCleanup is gated on ExitReason, which D16 introduces), D16 (ExitReason controls when the state file is removed), D17 (per-worktree iteration log preserves cross-invocation visibility for lessons-learned/post_issue_summary on resume), D19 (--fresh as the explicit-abandon affordance), D14 (prune excludes the active worktree).
...
- **Question:** Today `main.go:298` discards `RunResult` (`_ = workflow.Run(...)`). `RunResult` currently carries `IterationsRun int` and nothing else; the state-file-removal hook needs to know whether a run completed naturally, the user quit, or `breakLoopIfEmpty` triggered an exit. How is this signal carried out of `Run`?
...
### D17: Per-worktree iteration log

- **Question:** Today `iteration.jsonl` is a single file inside `<projectDir>/.pr9k/`, opened with `O_APPEND` and truncated only by the default workflow's `lessons-learned` finalize step. Cross-run resume causes multiple pr9k invocations to share a worktree. How should the iteration log behave across those invocations?
- **Decision:** Keep today's single-file, append-only model — but scope it to the **worktree** rather than to a single invocation. The path is `<worktree>/.pr9k/iteration.jsonl`. Every pr9k invocation that resumes into the worktree opens the same file with `O_WRONLY|O_CREATE|O_APPEND` and appends to whatever prior invocations left behind. The file's lifetime is the worktree's lifetime: `lessons-learned` truncates it at the end of finalization (today's behavior, unchanged), and the file is removed when the worktree is removed by autoCleanup or `pr9k worktree prune`. Each `IterationRecord` carries a new `invocation_stamp` field so a forensic reader can attribute records to a specific invocation without splitting them across files.
- **Rationale:** This is a reversal of an earlier draft of D17 that committed to per-invocation files (`iteration-<invocation-stamp>.jsonl`) with a stable alias at `iteration.jsonl`. User testing of that model surfaced the regression: when a user quits pr9k mid-run before `lessons-learned` runs, today's append-only model preserves the killed invocation's records into the resumed run — `lessons-learned` and `post_issue_summary` both see the full set. The per-invocation-file model would have hidden those records behind the alias (which only points at the current invocation), so the resumed run's `lessons-learned` would see only its own records and lose the killed invocation's forensic value. The user's stated requirement is "I want the worktree to also be able to see all of the iterations from a previous run, if `Lessons learned` step never executes" — which is satisfied exactly by the today-style append-only model. **Accepted trade-off (review R3, F35):** worktree-scoped append-only is *not* identical to today's in-place behavior — today's file accumulates only across iterations within one run, whereas the worktree's file accumulates across **every invocation** that resumes into the worktree, including invocations that worked on different issues. This means `post_issue_summary` (which reads the whole file with no `issue_id` filter) may include records from a prior, interrupted invocation's *unrelated* issues in the GitHub comment for the current issue. This is an expanded version of the pre-existing within-run quirk the script comment documents, not the same one. The trade-off is accepted because the forensic-value gain (cross-invocation visibility for `lessons-learned`) outweighs the comment-contamination cost (bounded by the next successful finalization), and a per-iteration `issue_id` filter in `post_issue_summary` is a candidate follow-up that would eliminate the contamination without affecting `lessons-learned`. `lessons-learned` is *designed* to look across iterations, and remains correct under the expanded scope.
- **Evidence:** `workflow/scripts/post_issue_summary` (reads the whole file each run, comment at line 4-6 documenting cross-iteration accumulation as expected); `workflow/prompts/lessons-learned.md` (line 1 `@.pr9k/iteration.jsonl`, line 5 "Read `.pr9k/iteration.jsonl`", line 8 truncation); `src/internal/workflow/iterationlog.go:47-49` (`O_WRONLY|O_CREATE|O_APPEND`, no rotation); user statement on the regression risk during plan revision.
- **Rejected alternatives:**
  - **Per-invocation files (`iteration-<invocation-stamp>.jsonl`) with a stable `iteration.jsonl` alias** — rejected on user feedback. It preserves invocation attribution at the filesystem level but loses cross-invocation visibility for `lessons-learned`/`post_issue_summary` on a resumed run, which is exactly the case where forensic value matters most. The append-only model with an `invocation_stamp` field on each record gives the same attribution without splitting the file.
  - Truncate `iteration.jsonl` at the start of every invocation — rejected because it loses prior-invocation history on resume (defeats the stated requirement).
...
- **Linked technical notes:** [T5](feature-technical-notes.md#t5-per-worktree-iteration-log)
- **Driven by findings:** V4 (resume validation); review F3 (consumer-update path); user revision after R2 (regression on the kill-before-lessons-learned path).
- **Dependent decisions:** D15 (cross-run resume is the reason the change applies); D13 (autoCleanup removes the file with the worktree).
- **Referenced in spec:** Outcome, Primary Flow (step 6), Edge Cases (iteration-log-spans-invocations row, post_issue_summary-includes-prior-invocations row), Coordinations (per-worktree iteration log row).
...
- **Decision:** A new `--fresh` boolean flag on the workflow runner. When passed, pr9k at startup runs the concurrent-run liveness check first: if the recorded PID is alive AND its binary matches pr9k's, `--fresh` refuses with the same "another pr9k appears to be running" error as a non-fresh start (review F7). Otherwise pr9k force-removes the worktree referenced in the state file (regardless of the current `worktrees.enabled` value — the state file's existence proves a prior `enabled: true` run created it; review F2), deletes that branch, and removes the state file. Then pr9k proceeds as a normal fresh start (creating a new worktree if `enabled: true`, proceeding in-place if `enabled: false`). The flag is a no-op for cleanup purposes if no state file exists; pr9k still prints a "Nothing to discard — proceeding as a fresh run" line.
- **Rationale:** Resume is automatic by default — that's the user's stated requirement. But there must be an opt-out for the case where the user wants to discard partial work (e.g., the prior run was on a now-dead branch, or the user is intentionally restarting). A CLI flag is the right surface: it's a per-invocation override, doesn't pollute config, and has a clear name. Two refinements followed from review: (1) `--fresh` cannot bypass the concurrent-run guard, because letting it destroy state belonging to a live process would damage the running run rather than recover from it; (2) the worktree removal is unconditional regardless of the current `worktrees.enabled` value, because anchoring removal to the current `enabled` value would orphan worktrees when the user toggles `enabled: false` between runs (review F2).
