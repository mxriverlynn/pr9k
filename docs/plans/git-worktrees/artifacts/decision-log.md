...
- **Decision:** Yes — `post_issue_summary` and `lessons-learned` both read `iteration.jsonl` (a hardcoded relative path) and would mix records from different invocations under the naive single-file model. Switch to **per-invocation files** named `iteration-<invocation-stamp>.jsonl`, with a **stable alias** at `iteration.jsonl` that always points to the current invocation's file (mechanic captured in T5). Each pr9k invocation writes to its own stamped file inside `<worktree>/.pr9k/` and refreshes the alias so the path the consumers already read points at the current run. Past invocations' stamped files persist alongside the worktree until autoCleanup or `pr9k worktree prune` removes the worktree. The default workflow's `lessons-learned` prompt is updated as part of this feature: the line that instructs Claude to "truncate `.pr9k/iteration.jsonl`" is removed — per-invocation files are cleaned up by autoCleanup/prune (the worktree-level cleanup paths), so the prompt-level truncation is redundant and would, after the alias change, only blank out the current invocation's records.
- **Rationale:** Validation finding V4 surfaced that the existing consumers cannot tolerate mixed records. Per-invocation files preserve forensic value (a prior invocation's file is directly readable). The naive expectation that consumers could simply use `RunStamp()` to find the current file was rejected (review F3) — `RunStamp()` is a Go-runtime value, not exported to subprocesses, and the existing consumers are shell scripts and prompts that hardcode `iteration.jsonl`. The stable-alias approach keeps every existing consumer working unchanged while delivering per-invocation forensic value. The mechanic (how the alias is realised — POSIX symlink, atomic-rename of a small index file, etc.) is captured in T5; the spec's behavioural commitment is "consumers read `iteration.jsonl` and always see the current invocation's records, with prior invocations' records preserved under stamped filenames alongside."
...
- **Linked technical notes:** [T5](feature-technical-notes.md#t5-stable-iteration-log-alias)
- **Driven by findings:** V4 (resume validation); review F3 (consumer-update path).
- **Dependent decisions:** D15 (cross-run resume is the reason the change is needed).
- **Referenced in spec:** Outcome, Primary Flow (step 6), Edge Cases (per-invocation accumulation row), Coordinations (iteration log alias row).
