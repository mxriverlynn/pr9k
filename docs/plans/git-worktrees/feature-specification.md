...
- Run-time observability is preserved: each pr9k invocation writes its own log file and its own per-invocation iteration log inside the worktree, and a stable alias `iteration.jsonl` always points to the current invocation's file so existing consumers (`post_issue_summary`, `lessons-learned`) keep working unchanged ([D3](artifacts/decision-log.md#d3-log-durability), [D17](artifacts/decision-log.md#d17-per-invocation-iteration-log-files), [T5](artifacts/feature-technical-notes.md#t5-stable-iteration-log-alias)).
...
6. pr9k completes startup against the worktree path: preflight runs, the run's log file is opened, and a per-invocation iteration log is opened at `<worktree>/.pr9k/iteration-<invocation-stamp>.jsonl`. A stable alias at `<worktree>/.pr9k/iteration.jsonl` always reflects the current invocation's iteration log, so that existing workflow-step consumers (`post_issue_summary`, `lessons-learned`, and any user-written script) continue to read the path they always have without modification ([D17](artifacts/decision-log.md#d17-per-invocation-iteration-log-files), [T5](artifacts/feature-technical-notes.md#t5-stable-iteration-log-alias)). Past invocations' iteration logs persist alongside, removed only by autoCleanup or `pr9k worktree prune`.
...
| Per-invocation iteration log accumulation | Each pr9k invocation writes a new `iteration-<invocation-stamp>.jsonl` file inside the worktree's `.pr9k/`. A stable alias `iteration.jsonl` always points to the current invocation's file, so existing consumers (`post_issue_summary`, `lessons-learned`) read the path they always have without modification. Past invocations' files persist until autoCleanup or `pr9k worktree prune` removes the worktree. ([D17](artifacts/decision-log.md#d17-per-invocation-iteration-log-files), [T5](artifacts/feature-technical-notes.md#t5-stable-iteration-log-alias)) |
...
| Active-run state file at `<primary>/.pr9k/active-run.json` | filesystem | Atomic-written at run start (fresh path only); read at every startup regardless of `worktrees.enabled`; removed on graceful end (Completed/LoopBroken) but kept on UserQuit and on hard kill. | Write happens once per fresh run, after preflight. Read happens once per pr9k startup, before subsystem construction. When `worktrees.enabled: false`, pr9k reads the file (to warn) but does not modify it. ([T4](artifacts/feature-technical-notes.md#t4-active-run-state-file-schema)) |
| Per-invocation iteration log alias at `<worktree>/.pr9k/iteration.jsonl` | filesystem | Maintained alongside the stamped per-invocation file `iteration-<invocation-stamp>.jsonl`; always reflects the current invocation's iteration log. | Created/updated atomically at logger initialization, before the first record is written. Survives across pr9k invocations only as a pointer to the current invocation; past invocations' stamped files persist alongside. ([D17](artifacts/decision-log.md#d17-per-invocation-iteration-log-files), [T5](artifacts/feature-technical-notes.md#t5-stable-iteration-log-alias)) |
...
- **OI-1:** The push-step change required by [D7](artifacts/decision-log.md#d7-first-push-sets-upstream-and-step-reorder) and the step reorder required by [D18](artifacts/decision-log.md#d18-push-before-close-step-ordering) are **in scope** for the worktree feature and ship with it (this is no longer the open question). The remaining open question is narrower: should the **in-place** workflow path (`worktrees.enabled: false`) also gain push-failure surfacing? Recommended answer: yes (failed pushes are dangerous regardless of mode). Non-blocking — the worktree feature can ship with the push step fixed and that fix taking effect in both modes by default; a separate decision could carve in-place mode out if there is a reason to.
...
- **Rounds completed:** 2 — see [artifacts/review-iteration-history.md](artifacts/review-iteration-history.md).
- **Team composition:**
...
- **Findings raised:** 29 major (F1–F29) and 6 minor (m1–m6) — see [artifacts/review-findings.md](artifacts/review-findings.md). All major findings were resolved in-spec by evidence; 1 was a YAGNI candidate (F15) replaced with a simpler version. None were deferred to the user.
...
- **Assumptions challenged across all passes:** branch naming consistency (slash vs hyphen); `--fresh` cross-config-flip behaviour; iteration-log consumer access path (`RunStamp()` was not actually exported to subprocesses); push-step prerequisite blocking-vs-non-blocking; concurrent-run TOCTOU race; primaryPath comparison semantics on macOS; prune walk-up false-positives; state-file warning persistence; cleanup ordering crash-resilience.
- **Consolidations made:** branch-name standardisation across spec/decision-log/tech-notes; mechanics-leaking-into-spec sweep on Primary Flow and Alternate Flow bodies; Edge Cases table extended with 12 new rows for previously implicit failure modes.
- **Ambiguities resolved, and how:** F2/F7 `--fresh` semantics rewritten (unconditional removal + concurrent-guard refusal); F3/T5 iteration-log access via stable alias rather than `RunStamp()` derivation; F19 prune walk-up disambiguated against legitimately-named directories; F20 path-comparison rule added (canonical realpath); F21 uncommitted-changes scope spelled out (modified + untracked); F23 schema-version misclassification cleaned up; F29 cleanup ordering specified for crash-resilience.
- **Technical notes added/edited:** 1 added (T5 — stable iteration-log alias), 2 edited (T2 — branch-prefix load-bearing claim corrected and naming aligned to hyphen; T4 — resume validation set extended, path-comparison semantics added, schema-version handling rewritten) — see [artifacts/feature-technical-notes.md](artifacts/feature-technical-notes.md).
- **Open items remaining:** 2 — OI-1 (in-place push-failure surfacing — non-blocking), OI-2 (`review_verdict` path consistency — non-blocking). Neither blocks implementation.
