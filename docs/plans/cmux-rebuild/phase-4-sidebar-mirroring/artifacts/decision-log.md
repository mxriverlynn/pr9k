...
- **Evidence:**
  ...
  - `cmux set-status --help` and `cmux set-progress --help` output on cmux 0.64.7 (transcribed in the planning conversation and reproducible against `/opt/homebrew/bin/cmux`).
  ...
  - The existing `internal/cmuxctl.CmuxClient` interface (`src/internal/cmuxctl/client.go`) is the natural extension point for the additional methods.
...
- **Decision:** pr9k uses a single stable key for its step status entry, prefixed `pr9k.` (specifically `pr9k.step` in the implementation, but the spec-level commitment is "stable and pr9k-prefixed"). pr9k does not list, inspect, or mutate any pill whose key is not `pr9k.*`. The progress bar is one-per-workspace by cmux's contract, so it does not need a key.
...
### D6: Sidebar entries are cleared on every run-end path

- **Question:** when does pr9k clear the sidebar entries?
- **Decision:** the orchestrator clears both entries (the `pr9k.step` status entry and the progress bar, when it was pushed) as part of its shutdown sequence on every normal run-end path — successful completion, unrecoverable abort, and operator-initiated quit. The clear calls are best-effort under [D5](#d5-non-timeout-sidebar-errors-are-logged-and-the-run-continues-timeouts-remain-fatal-per-parent-d15): an error is logged but does not change the orchestrator's exit code.
- **Rationale:** without a clear on run-end, the operator's sidebar would show stale pr9k state pointing at a workspace that is no longer doing anything — actively misleading. Clearing on every normal exit path (rather than only success) is the simplest invariant for the operator and the implementer.
- **Evidence:**
...
- **D10:** Sidebar pushes happen on the same cadence as the header pane's pushes — one sidebar push per header push for whichever of the two entries changed. — Follows directly from parent D5 ("update on the same cadence as the header pane"). — Referenced in spec: Outcome, Primary Flow.
