...
**Note on Assumption A1 (start here before any wiring).** A1 — that the `internal/ui` renderers (`StatusHeader`, log panel, `KeyHandler`, footer state machine) are usable from the `cmux-pane` sub-command entry points without standard-mode Bubble Tea program-loop dependencies — is unverified. Inspect the renderer constructors first. If A1 proves false (renderers need bespoke wrappers), stop and surface the plan's D-2 decomposition revisit criterion before continuing; this is the HITL judgment gate for this work item.
