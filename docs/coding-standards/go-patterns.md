...
**Incompatible schema version** — the file parses but has an unknown version number. Rename it to `<name>.incompatible-<unixnano>` and return `(nil, nil)`. Same outcome: fresh start, file preserved.
...
```go
// Good — type is string; value is the JSON/log representation directly.
...
// Usage in a switch — works without a String() method.
...
Choose `type X string` when:
- The values appear in JSON, log files, or user-facing output and must be human-readable without a formatting step.
- The set of values is small and stable (not generated by an external system).
- Exhaustive switch detection (via a linter or `panic` fallback — see "Use exhaustive switches" in this file) is still required.
