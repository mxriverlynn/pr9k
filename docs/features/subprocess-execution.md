...
- `RunSandboxedStep` executes a command inside a Docker sandbox, installing a terminator closure and using explicit empty stdin to prevent raw-mode keyboard inheritance; the cidfile is cleaned up (ENOENT-tolerant) after the step exits. When `opts.CaptureMode == CaptureResult`, a `claudestream.Pipeline` is constructed and the claude-aware stdout path is activated (no 256 KB line cap; each line is fed to `pipeline.Observe`); after the subprocess exits, `lastStats` is populated from the pipeline's aggregator and the aggregator error is checked before the process exit code
...
- **Pipeline non-nil (claude path):** `bufio.Reader.ReadString('\n')` loop with no line-length cap. Each raw line is passed to `pipeline.Observe`; returned display lines are forwarded via `sendLine` and logged. `lastCapture` is not set here — `RunSandboxedStep` sets it from `pipeline.Aggregator().Result()` after the pipeline is closed.
