...
// --- TP-213: WorktreeDetails ---

// TP-213-006: WorktreeDetails returns three lines: worktree path, branch, count.
func TestWorktreeDetails_TypicalValues(t *testing.T) {
	lines := WorktreeDetails("/tmp/pr9k-2026-05-08-114530.123", "pr9k-2026-05-08-114530.123", 3)
	if len(lines) != 3 {
		t.Fatalf("expected 3 lines, got %d", len(lines))
	}
	wantPath := "worktree: /tmp/pr9k-2026-05-08-114530.123"
	wantBranch := "branch: pr9k-2026-05-08-114530.123"
	wantCount := "pr9k worktrees: 3"
	if lines[0] != wantPath {
		t.Errorf("lines[0] = %q, want %q", lines[0], wantPath)
	}
	if lines[1] != wantBranch {
		t.Errorf("lines[1] = %q, want %q", lines[1], wantBranch)
	}
	if lines[2] != wantCount {
		t.Errorf("lines[2] = %q, want %q", lines[2], wantCount)
	}
}

// TP-213-007: WorktreeDetails with count zero.
func TestWorktreeDetails_ZeroCount(t *testing.T) {
	lines := WorktreeDetails("/tmp/wt", "my-branch", 0)
	if len(lines) != 3 {
		t.Fatalf("expected 3 lines, got %d", len(lines))
	}
	if lines[2] != "pr9k worktrees: 0" {
		t.Errorf("lines[2] = %q, want %q", lines[2], "pr9k worktrees: 0")
	}
}

// TP-213-008: WorktreeDetails with count one.
func TestWorktreeDetails_OneWorktree(t *testing.T) {
	lines := WorktreeDetails("/path/wt-abc", "wt-abc", 1)
	if lines[2] != "pr9k worktrees: 1" {
		t.Errorf("lines[2] = %q, want %q", lines[2], "pr9k worktrees: 1")
	}
}
