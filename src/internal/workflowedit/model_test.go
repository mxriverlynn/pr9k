...
// applyMsg delivers an arbitrary tea.Msg to the model.
...
// TestModel_Mode_21 — DialogSaveInProgress + saveCompleteMsg → quit flow
func TestModel_Mode_21_DialogSaveInProgress_SaveComplete_EntersQuitFlow(t *testing.T) {
	...
	m.saveInProgress = true
	m.pendingQuit = true
	// Successful save arrives
	_, cmd := m.Update(saveCompleteMsg{result: workflowio.SaveResult{Kind: workflowio.SaveErrorNone}})
	if cmd == nil {
