...
// TestSaveComplete_WithPendingQuit_ReentersQuitFlow — save+pendingQuit → DialogQuitConfirm
// (D-PR2-10: re-routes through handleGlobalKey; always-confirm before exit).
func TestSaveComplete_WithPendingQuit_ReentersQuitFlow(t *testing.T) {
	...
	next, _ := m.Update(saveCompleteMsg{result: workflowio.SaveResult{
		Kind:     workflowio.SaveErrorNone,
		Snapshot: snapshot,
	}})
	got := next.(Model)
