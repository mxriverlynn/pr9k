...
//
// Sequence:
//  1. os.MkdirAll(projectDir+"/.pr9k") — creates the umbrella dir for
//     iteration.jsonl and .pr9k/logs/ on first run. Pre-created under the
//     host UID before the container runs, to avoid a chmod fight when the
//     container writes via the sandbox's UID mapping.
//  2. CheckProfileDir(profileDir) — only when hasClaudeSteps is true.
//  3. CheckDocker(p) — only when hasClaudeSteps is true.
